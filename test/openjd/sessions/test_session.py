...
class TestEnvironmentVariablesInTasks_2023_09:
        ...
        environment = Environment_2023_09(
            ...
            script=EnvironmentScript_2023_09(
                actions=EnvironmentActions_2023_09(
                    onEnter=Action_2023_09(
                        command=sys.executable,
                        args=["-c", "print('openjd_redacted_env: PASSWORD=secret123')"],
                    )
                )
            ),
        )
        ...
        environment = Environment_2023_09(
            ...
            script=EnvironmentScript_2023_09(
                actions=EnvironmentActions_2023_09(
                    onEnter=Action_2023_09(
                        command=sys.executable,
                        args=["-c", "print('openjd_redacted_env: API_KEY=abc123def456')"],
                    )
                )
            ),
        )
        ...
        script = StepScript_2023_09(
            actions=StepActions_2023_09(
                onRun=Action_2023_09(
                    ...
                    args=[
                        "-c",
                        "import os; print(f'API_KEY={os.environ[\"API_KEY\"]}')",
                    ],
                )
            ),
        )
    ...
    def test_session_with_enabled_extensions(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with Session(
            ...
            # THEN
            assert session._enabled_extensions == enabled_extensions
    ...
    def test_session_with_no_extensions(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with Session(
            ...
            # THEN
            assert session._enabled_extensions == []
        ...
        environment = Environment_2023_09(
            ...
            script=EnvironmentScript_2023_09(
                actions=EnvironmentActions_2023_09(
                    onEnter=Action_2023_09(
                        command=sys.executable,
                        args=["-c", "print('openjd_redacted_env: TOKEN=secret-token')"],
                    )
                )
            ),
            variables={"TOKEN": "public-token"},
        )
        ...
        script = StepScript_2023_09(
            actions=StepActions_2023_09(
                onRun=Action_2023_09(
                    ...
                    args=[
                        "-c",
                        "import os; print(f'TOKEN={os.environ[\"TOKEN\"]}')",
                    ],
                )
            ),
        )
    ...
    def test_def_via_redacted_env_with_extension(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with Session(
            session_id=session_id,
            job_parameter_values=job_params,
            enabled_extensions=["REDACTED_ENV_VARS"],  # Enable the extension
        ) as session:
            session.enter_environment(environment=environment)
            while session.state == SessionState.RUNNING:
                time.sleep(0.1)
